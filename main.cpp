...
int main() {
  ...
  sf::Text credit(font, "Made by Angad", 24);
  credit.setPosition({50.f, 550.f});
  ...
  while (window.isOpen()) {
    ...
    if (leftPaddle.getPosition().y < 0) {
      ...
      leftPaddle.setPosition(
          {leftPaddle.getPosition().x, 600 - leftPaddle.getSize().y});
    }
    ...
    if (rightPaddle.getPosition().y < 0) {
      ...
      rightPaddle.setPosition(
          {rightPaddle.getPosition().x, 600 - rightPaddle.getSize().y});
    }
    ...
    if (gameStarted) {
      ...
      if (ball.getPosition().y - ball.getRadius() <= 0 ||
          ball.getPosition().y + ball.getRadius() >= 600) {
...
bool isColliding(const sf::RectangleShape &rect,
  ...
  float closestX = std::clamp(circleCenter.x, rectBounds.position.x,
                              rectBounds.position.x + rectBounds.size.x);
  float closestY = std::clamp(circleCenter.y, rectBounds.position.y,
                              rectBounds.position.y + rectBounds.size.y);
